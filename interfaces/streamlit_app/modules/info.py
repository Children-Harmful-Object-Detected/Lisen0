...
def draw_pose_on_image(img, r_pose, names):
    ...
    for kp, box in zip(r_pose.keypoints, r_pose.boxes):
        ...
        # Use fixed colors for pose bounding boxes since Adult/Child label is removed
        box_color = (0, 165, 255) # Orange-ish for general person bounding box for clarity
        
        pose_boxes.append((x1, y1, x2, y2, None, box_color)) # No label appended
    ...
    for (x1, y1, x2, y2, label, box_color) in pose_boxes: # 'label' is now None, so don't use it
...
def show_frame_labeling_tab(MODEL_YOLO, selected_video):
    ...
    yolo_pose, yolo_box, transformer = load_models(MODEL_YOLO)
   ...
   # =========================================================
    ...
    # -----------------------------------
    # 🟥 Box Detection (best.pt)
    # -----------------------------------
    st.markdown("### 🟥 Box Detection (best.pt)")
